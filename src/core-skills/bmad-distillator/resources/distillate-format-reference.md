...
- Entry: TypeScript sources under `cli/` -> compile to `cli/dist/bmad-cli.js` (Commander.js) -> `core/installer`
...
- Modules: built-in `core` and `bmm` from this repository (no remote registry install path)
