...
export class ArtifactResolver {
  ...
  getTargetPath(projectDir, ide, artifact, platformConfig, artifactsDir) {
    ...
    const actualArtifactsDir = artifactsDir || path.join(projectDir, '.IDE');
  ...
  isValidArtifactLocation(sourceDir, projectDir, artifactType, artifactsDir) {
    const typeRootDir = (artifactsDir ? path.join(artifactsDir, artifactType) : path.join(projectDir, '.IDE', artifactType));
    const artifactsRootDir = artifactsDir || path.join(projectDir, '.IDE');
