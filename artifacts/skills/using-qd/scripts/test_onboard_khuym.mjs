...
function runSessionStartHook(root, payload = { cwd: root }, env = {}) {
  const hookPath = path.join(root, ".codex", "hooks", "_qd_session_start.mjs");
...
test("applyRepo creates full repo onboarding with node-based hooks", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "qd-onboard-"));
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(root, "._qd", "state.json")));
    assert.ok(fs.existsSync(path.join(root, ".codex", "hooks", "_qd_session_start.mjs")));
    assert.ok(fs.existsSync(path.join(root, ".codex", "_qd_status.mjs")));
    assert.ok(fs.existsSync(path.join(root, ".codex", "_qd_state.mjs")));
    assert.ok(fs.existsSync(path.join(root, ".codex", "_qd_dependencies.mjs")));
    assert.match(
      fs.readFileSync(path.join(root, ".codex", "hooks.json"), "utf8"),
      /node \.codex\/hooks\/_qd_session_start\.mjs/,
    );
    assert.equal(
      JSON.parse(fs.readFileSync(path.join(root, "._qd", "state.json"), "utf8")).phase,
      "idle",
    );
  } finally {
...
test("checkRepo flags stale python hook commands and legacy hook files", () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(hooksDir, "_qd_session_start.py"), "# legacy\n", "utf8");

    const result = checkRepo(root);

    assert.equal(result.status, "needs_onboarding");
    assert.ok(result.actions.includes("install__qd_hook_entries"));
    assert.ok(result.actions.includes("sync__qd_hook_scripts"));
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});

test("installed _qd_status script reports onboarding and state", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "qd-onboard-"));
...
test("installed _qd_status reports gkg readiness for a supported indexed repo", async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "qd-gkg-"));
...
test("installed _qd_status text distinguishes missing commands from missing MCP config", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "qd-onboard-"));

  try {
    applyRepo(root, false);
    const skillsRoot = path.join(root, "plugins", "qd", "skills");
    ...
    const stdout = execFileSync("node", [path.join(root, ".codex", "_qd_status.mjs")], {
      cwd: root,
      encoding: "utf8",
    });
    ...
    assert.match(stdout, /Affects: exploringalpha/);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
...
test("dependency report distinguishes dependency-free packaged skills from uncovered ones", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "qd-coverage-"));
  const skillsRoot = path.join(root, "plugins", "qd", "skills");
  ...
  try {
    ...
    assert.deepEqual(report.uncovered_skills.map((skill) => skill.skill_name), ["exploringbeta"]);

    const alpha = report.skills.find((skill) => skill.skill_name === "exploringalpha");
    const beta = report.skills.find((skill) => skill.skill_name === "exploringbeta");
...
test("_qd_status text surfaces uncovered packaged skills instead of skipping them", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "qd-coverage-"));
  ...
  try {
    ...
    assert.match(stdout, /exploringbeta/);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
...
test("checkRepo promotes missing dependency data into an operator-facing warning summary", () => {
  ...
  try {
    ...
    assert.deepEqual(warning.affected_skills, ["exploringalpha"]);
    assert.match(warning.message, /Dependency warning:/);
    assert.match(warning.message, /exploringalpha/);
...
test("session-start hook emits dependency warning with command-vs-MCP split when dependencies are missing", () => {
  ...
  try {
    ...
    assert.match(context, /exploringalpha/);
...
test("session-start hook surfaces gkg readiness guidance for supported repos that are not ready", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "qd-gkg-hook-"));
...
test("entry surfaces share the same missing-command vs missing-MCP wording boundary", () => {
  ...
  try {
    ...
    const scoutText = execFileSync("node", [path.join(root, ".codex", "_qd_status.mjs")], {
      cwd: root,
      encoding: "utf8",
    });
    ...
    assert.match(scoutText, /Affects: exploringalpha/);
    ...
    assert.match(onboardingWarning.message, /Affected skills: exploringalpha/);
...
test("dependency helper marks missing command and missing mcp_server dependencies", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "qd-deps-"));
  const skillsRoot = path.join(root, "plugins", "qd", "skills");
...
test("dependency helper respects declared MCP config_sources and can use packaged manifests", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "qd-mcp-sources-"));
  const pluginRoot = path.join(root, "plugins", "qd");
  const skillsRoot = path.join(root, "plugins", "qd", "skills");
...
test("dependency helper still accepts legacy root-level plugin MCP manifests", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "qd-mcp-legacy-"));
  const pluginRoot = path.join(root, "plugins", "qd");
  const skillsRoot = path.join(root, "plugins", "qd", "skills");
...
test("packaged Khuym inventory stays fully covered and the docs explain the declaration contract", () => {
  ...
  const pluginMcp = JSON.parse(
    fs.readFileSync(path.join(LOCAL_REPO_ROOT, "plugins", "qd", ".mcp.json"), "utf8"),
  );
  ...
  assert.match(
    skillText,
    /bash scripts\/check-markdown-links\.sh plugins\/qd\/skills\/using-qd\/SKILL\.md/,
  );
