...
const MANAGED_HOOK_FILENAMES = [
  ...
  "_qd_pre_tool_use.mjs",
  "_qd_stop.mjs",
];
const LEGACY_HOOK_FILENAMES = [
  "_qd_session_start.py",
  "_qd_pre_tool_use.py",
  "_qd_stop.py",
];
...
export function checkRepo(repoRoot) {
  ...
  if (!agentsExists) {
    ...
    actions.push("append__qd_managed_block_to_AGENTS.md");
  }
  ...
  if (!configText) {
    ...
    if (compactPromptConflict) {
      ...
      actions.push("install__qd_compact_prompt");
    }
  }
  ...
  if (hooksNeedMerge) {
    actions.push("install__qd_hook_entries");
  }

  if (hookScriptsNeedUpdate(repoRoot)) {
    actions.push("sync__qd_hook_scripts");
  }

  if (supportScriptsNeedUpdate(repoRoot)) {
    actions.push("sync__qd_support_scripts");
  }

  if (needsStateUpdate(repoRoot)) {
    actions.push("write_._qd/state.json");
  }

  if (onboarding.plugin_version !== pluginVersion) {
    actions.push("write_._qd/onboarding.json");
  }
