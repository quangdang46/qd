...
/**
 * YAML Formatter and Linter for QD Framework
...
async function formatYamlContent(content, filename) {
  ...
  try {
    ...
    if (fixedContent !== content) {
      console.log(chalk.blue(`[FIXED] Applied YAML fixes to ${filename}`));
    }
    ...
    console.error(chalk.red(`[ERROR] YAML syntax error in ${filename}:`), error.message);
    console.error(chalk.yellow(`[TIP] Try manually fixing the YAML structure first`));
    return null;
  }
}
...
async function processMarkdownFile(filePath) {
  ...
  if (newContent !== content) {
    modified = true;
    console.log(chalk.blue(`[FIXED] Added 'text' type to untyped code blocks in ${filePath}`));
  }
  ...
  while ((match = yamlBlockRegex.exec(newContent)) !== null) {
    ...
    if (formatted !== null) {
      ...
      if (trimmedFormatted !== yamlContent) {
        modified = true;
        console.log(chalk.green(`[OK] Formatted YAML in ${filePath}`));
      }
...
async function lintYamlFile(filePath) {
  ...
  try {
    ...
    console.error(chalk.red(`[ERROR] YAML lint error in ${filePath}:`));
...
async function main() {
  ...
  for (const filePath of allFiles) {
    ...
    try {
      ...
      console.error(chalk.red(`[ERROR] Error processing ${filePath}:`), error.message);
      hasErrors = true;
    }
  }

  if (hasChanges) {
    console.log(chalk.green(`\n[DONE] YAML formatting completed! Modified ${filesProcessed.length} files:`));
    for (const file of filesProcessed) console.log(chalk.blue(`  - ${file}`));
  }

  if (hasErrors) {
    console.error(chalk.red('\n[ERROR] Some files had errors. Please fix them before committing.'));
    process.exit(1);
  }
}
